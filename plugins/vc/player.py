...
Play dan Kontol membutuhkan sebuah akses
...
USERBOT_HELP = f"""{emoji.LABEL}  **Perintah Umum (member)**:
__untuk member grup chat__
__dengan / (slash) atau ! (exclamation mark)__

\u2022 **/play**  reply dengan lagu untuk play/antrian, atau lihat playlist
\u2022 **/current**  lihat yang sedang diputar
\u2022 `!help`  lihat untuk bantuan


{emoji.LABEL}  **Admin Commands**:
__Berlaku untuk userbot itu sendiri dan kontaknya__
__Mulai dengan ! (exclamation mark)__

\u2022 `!skip` [n] ...  skip lagu berikutnya >= 2
\u2022 `!join`  join ke OS
\u2022 `!leave`  keluar dari OS
\u2022 `!vc`  check dengan VC telah bergabung
\u2022 `!stop`  stop putar
\u2022 `!replay`  memutar lagu kembali
\u2022 `!clean`  hapus sampah RAW PCM files
\u2022 `!pause` pause
\u2022 `!resume` resume
\u2022 `!mute`  mute VC userbot
\u2022 `!unmute`  unmute VC userbot
"""
...
async def network_status_changed_handler(gc: GroupCall, is_connected: bool):
    if is_connected:
        ...
        await send_text(f"{emoji.CHECK_MARK_BUTTON} Bergabung ke voice chat")
    else:
        await send_text(f"{emoji.CROSS_MARK_BUTTON} Keluar dari voice chat")
        mp.chat_id = None
...
async def play_track(client, m: Message):
    ...
    if m.audio:
        if m.audio.duration > (DURATION_AUTOPLAY_MIN * 60):
            reply = await m.reply_text(
                f"{emoji.ROBOT} durasi audio telalu lama "
        ...
        if m_audio.audio.duration > (DURATION_PLAY_HOUR * 60 * 60):
            reply = await m.reply_text(
                f"{emoji.ROBOT} durasi audio telalu lama "
                f"{str(DURATION_PLAY_HOUR)} Batas jam tidak dapat di tambahkan ke playlist"
            )
            await _delay_delete_messages((reply,), DELETE_DELAY)
            return
    else:
    ...
    if playlist and playlist[-1].audio.file_unique_id \
        ...
        reply = await m.reply_text(f"{emoji.ROBOT} sudah ditambahkan")
    ...
    if len(playlist) == 1:
        m_status = await m.reply_text(
            f"{emoji.INBOX_TRAY} downloading dan transcoding..."
        )
...
async def show_current_playing_time(_, m: Message):
    ...
    playlist = mp.playlist
    if not start_time:
        reply = await m.reply_text(f"{emoji.PLAY_BUTTON} tidak ada")
        await _delay_delete_messages((reply, m), DELETE_DELAY)
        return
...
async def skip_track(_, m: Message):
    ...
    if len(m.command) == 1:
        ...
        try:
            ...
            reply = await m.reply_text(f"{emoji.NO_ENTRY} kesalahan input",
                                       disable_web_page_preview=True)
        await _delay_delete_messages((reply, m), DELETE_DELAY)
...
async def join_group_call(client, m: Message):
    ...
    if group_call.is_connected:
        await m.reply_text(f"{emoji.ROBOT} sudah join di voice chat")
        return
...
async def list_voice_chat(client, m: Message):
    ...
    if group_call.is_connected:
        ...
        reply = await m.reply_text(
            f"{emoji.MUSICAL_NOTES} **saat ini sedang dalam voice chat**:\n"
            f"- **{chat.title}**"
        )
    else:
...
async def stop_playing(_, m: Message):
    ...
    reply = await m.reply_text(f"{emoji.STOP_BUTTON} stop memutar")
...
async def resume_playing(_, m: Message):
    ...
    reply = await m.reply_text(f"{emoji.PLAY_OR_PAUSE_BUTTON} Memutar",
                               quote=False)
...
async def clean_raw_pcm(client, m: Message):
    ...
    reply = await m.reply_text(f"{emoji.WASTEBASKET} hapus {count} files")
    await _delay_delete_messages((reply, m), DELETE_DELAY)
...
async def mute(_, m: Message):
    ...
    reply = await m.reply_text(f"{emoji.MUTED_SPEAKER} bisukan")
    await _delay_delete_messages((reply, m), DELETE_DELAY)
...
async def unmute(_, m: Message):
    ...
    reply = await m.reply_text(f"{emoji.SPEAKER_MEDIUM_VOLUME} jalankan")
    await _delay_delete_messages((reply, m), DELETE_DELAY)
